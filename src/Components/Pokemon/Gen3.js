...
import { getPokemon3, getPokemonRandomArray3 } from '../../lib/api'
...
class Gen3 extends React.Component {
  ...
  async componentDidMount() {
    const pokemonListResponse = await getPokemon3()
    ...
    const pokemonArrayResponse = await getPokemonRandomArray3()
      ...
      <section>
        ...
        <div>
          {this.state.arrayNames.map(obj => {
            return ( 
              obj.correct ? 
                <Link to="/WinPage3" className={obj.classNames} id = "links">{this.capitalize(obj.name)}</Link>
                :
                <Link to="/LosePage3" className={obj.classNames} id = "links">{this.capitalize(obj.name)}</Link>
            )
          })}
        </div>
      </section>
